...
X_train, X_test, y_train, y_test = train_test_split( # Training-set und Test-set, wobei X_train = Eingabe-Matrix,
    ...
    X, y, test_size=0.2, random_state=77 # 0.2 = 20% der Daten für Test und damit 80 % für Training, random_state = 42
...
clf = RandomForestClassifier( #Classifer-Modell
...
dump(clf, "model.pkl")  # Serialisierung mit joblib(um unser Modell zu speichern und zu laden), damit unser
# training-Modell(RandomForestClassifier Entität) als .pkl-Datei gespeichert wird
print("Modell wurde trainiert und unter 'model.pkl' gespeichert.")
